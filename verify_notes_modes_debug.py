...
def run():
    with sync_playwright() as p:
        ...
        # Wait and take screenshot for debugging if it fails
        try:
            page.wait_for_selector(".notes-modal-view:not(.hidden)", timeout=5000)
        except Exception:
            print("Failed to find view mode, taking debug screenshot")
            page.screenshot(path="debug_view_mode.png")
            raise
